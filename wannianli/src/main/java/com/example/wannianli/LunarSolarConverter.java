...
public class LunarSolarConverter {
    ...
    public static Lunar converterDate(Solar solar) {
        return converterDate(solar.date.getTime());
    }
    ...
    public static Lunar converterDate(long timeInMillis) {
        if (timeInMillis < MIN_TIME_MILLIS || timeInMillis > MAX_TIME_MILLIS) {
            throw new RuntimeException("日期超出农历计算范围");
        }
        Lunar lunar = new Lunar();
        // 距离起始日期间隔的总天数 间隔天数和目标日期差一天 所以加上一天
        long offset = (timeInMillis - MIN_TIME_MILLIS) / (24 * 60 * 60 * 1000);
        // 默认农历年为1901年，且由此开始推算农历年份
        int lunarYear = 1900;
        while (true) {
     ...
     * @return 该农历年某月的天数
     */
    public static int getLunarDays(int lunarYear, int month) {
