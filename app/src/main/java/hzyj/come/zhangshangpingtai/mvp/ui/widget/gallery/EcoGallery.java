package hzyj.come.zhangshangpingtai.mvp.ui.widget.gallery;
...
public class EcoGallery extends EcoGalleryAbsSpinner implements GestureDetector.OnGestureListener {
    ...
    public EcoGallery(Context context, AttributeSet attrs, int defStyle) {
        ...
        int index = a.getInt(R.styleable.EcoGallery_mygravity, -1);
        if (index >= 0) {
            setGravity(index);
        }
        ...
        int spacing = a.getDimensionPixelOffset(R.styleable.EcoGallery_myspacing, 0);
        setSpacing(spacing);

        float unselectedAlpha = a.getFloat(R.styleable.EcoGallery_unselectedAlpha, 0.8f);
        ...
        if (mSelectedScale != 1.f || mUnselectedAlpha != 1.f) {
            ...
            if (offsetCenter < childWidth) {
                ...
                if (mSelectedScale != 1.f) {
                    ...
                    child.setPivotY(childHeight /2.f);
